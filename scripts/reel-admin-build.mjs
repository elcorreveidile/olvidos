// Build del reel "tour guiado" del admin (Equipo Editor). Como reel-clip-build,
// pero además inyecta las ESCENAS y META desde un JSON en la plantilla motor
// (reel-admin.template.html). Renderiza frame a frame con Chromium y codifica
// el mp4 con h264-mp4-encoder (WASM).
//
// Uso:
//   RAW_DIR=footage-reel/raw-admin-desktop SCENES=scripts/scenes-panel.json \
//   TEMPLATE=scripts/reel-admin.template.html \
//   OUT=footage-reel/clip-admin-panel.mp4 PORTADA=footage-reel/portada-admin-panel.png \
//   PORTADA_T=1.2 [PROBE=1] node scripts/reel-admin-build.mjs
...
const RAW = process.env.RAW_DIR || join(ROOT, "footage-reel/raw-admin-desktop");
const TEMPLATE = process.env.TEMPLATE || join(ROOT, "scripts/reel-admin.template.html");
const SCENES = process.env.SCENES || join(ROOT, "scripts/scenes-panel.json");
const OUT = process.env.OUT || join(ROOT, "footage-reel/clip-admin.mp4");
const PORTADA = process.env.PORTADA || join(ROOT, "footage-reel/portada-admin.png");
const PORTADA_T = Number(process.env.PORTADA_T || 1.2);
const FPS = Number(process.env.FPS || 30);
const QP = Number(process.env.QP || 24);
...
console.log("escenas:", SCENE_LIST.length, "->", SCENE_LIST.map((s) => s.img).join(", "));
...
const reelPath = join(ROOT, "footage-reel", basename(OUT).replace(/\.mp4$/, ".html"));
...
if (PROBE) {
  const times = (process.env.PROBE_TIMES || "0,1600,3000,5200,7000,9200,11000,13000,15000,17000").split(",").map(Number);
  for (const t of times) {
    await page.evaluate((tt) => window.renderAt(tt), t);
    await page.evaluate(() => new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r))));
    writeFileSync(join(ROOT, "footage-reel/probe", `a${String(t).padStart(5, "0")}.png`), await shot());
  }
  console.log("PROBE listo -> footage-reel/probe"); await browser.close(); process.exit(0);
}
